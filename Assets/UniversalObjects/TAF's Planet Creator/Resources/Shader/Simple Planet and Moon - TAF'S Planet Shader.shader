...
Shader "TAF's Planet Shader/Planet and Moon - Simple" {
    Properties {
        ...
        _RimColor ("RimColor", Color) = (0.3074719,0.5414053,0.8897059,1)
...
///////// Gloss:
                ...
                float gloss = (_Surface_var.a*_GlossPower);
                float perceptualRoughness = 1.0 - (_Surface_var.a*_GlossPower);
...
////// Specular:
                ...
                float3 specularColor = ((_Surface_var.a*_SpecPower)*_SpecColor.rgb).r;
                float specularMonochrome;
                float3 diffuseColor = _Surface_var.rgb; // Need this for specular when using metallic
...
////// Emissive:
                float3 node_9067 = (pow(1.0-max(0,dot(normalDirection, viewDirection)),_RimThickness)*_RimPower*max(0,dot(i.normalDir,lightDirection))*_RimColor.rgb);
                float3 node_8383 = (node_9067+node_9067);
                float3 emissive = (node_8383+node_8383);
/// Final Color:
